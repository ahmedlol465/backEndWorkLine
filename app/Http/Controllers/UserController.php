    ...
    public function verifyEmail(Request $request)
{
    ...
    public function login(Request $request)
    {
        ...
        if($request->type){
            ...
            //     'name' => $createUser->name, // include only necessary fields
            // ],
            //     'token' => $token,
            // ], 200);
    ...
    public function UserData(Request $request)
    {
        // Validate input - Remove userId from validation rules
        $validator = Validator::make($request->all(), [
            'specialist' => 'required|string|max:255',
            'jobTitle' => 'required|string|max:255',
            'description' => 'required|string',
        ...
        // Create new user data record using the authenticated user's ID
        $userData = UserData::create([
    ...
    public function UserWork(Request $request)
    {
        ...
        $imageNameThumb = Str::random(32) . '.' . $request->thumbnail->getClientOriginalExtension(); // Use $request->thumbnail for thumbnail
        Storage::disk('public')->put($imageNameThumb, file_get_contents($request->thumbnail)); // Use $request->thumbnail for thumbnail

        // Create new user work record using the authenticated user's ID
        $userWork = UserWork::create([
            'userId' => $userId, // Use $userId here
            'workTitle' => $request->workTitle,
            'workDescription' => $request->workDescription,
    ...
    public function updateProfile(Request $request)
        ...
        $user = auth()->user();
        $user->load(['userData', 'userWorks', 'userStatistics','userServices', 'userProjects']);
        return response()->json([
            'message' => 'Profile updated successfully',
            'user' => $user, // Or use a Resource to format the response nicely
        ], 200);
    ...
    public function ResetPassword(Request $request)
        ...
        public function sendResetLinkEmail(Request $request)
        {
            ...
            $token = Str::random(60); // Or use a more robust token generation method
            $password_reset_tokens = password_reset::create([
                'email' => $user->email,
                'code' => $token,
            ]);

            // Build reset link URL (replace with your frontend URL)
            // $resetLink = config('app.frontend_url') . '/reset-password?token=' . $token . '&email=' . urlencode($user->email); // Example frontend URL

            // Send email with reset link
            // Mail::to($user->email)->send(new ForgotPasswordMail($user, $resetLink)); // Create a Mailable class ForgotPasswordMail
            $email = $request->input('email');
                    $message = "Welcome to our platform!";
                    $subject = "Welcome to Our Service";
            ...
            return response()->json(['message' => 'If an account with this email exists, a password reset link has been sent to your email address.'], 200);
        }
